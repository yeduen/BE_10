...
def play_game():
    ...
    # 초기 카드 이미지 출력
    player_images = [card_images[(card['rank'], card['suit'])] for card in player_hand]  # 괄호 수정
    banker_images = [card_images[(card['rank'], card['suit'])] for card in banker_hand]  # 괄호 수정
    update_card_display()

    # 플랍(3장의 커뮤니티 카드)
    for _ in range(3):
        community_cards.append(deck.pop())
    community_images = [card_images[(card['rank'], card['suit'])] for card in community_cards]  # 괄호 수정
    update_card_display()

    # 턴(1장의 커뮤니티 카드)
    community_cards.append(deck.pop())
    community_images.append(card_images[(community_cards[-1]['rank'], community_cards[-1]['suit'])])  # 괄호 수정
    update_card_display()
